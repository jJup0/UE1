...
\\[0.5 em]
If too many $p$ will be used, the efficiency will turn into:
\\[0.5 em]
...
\answer{2}
We would expect the algorithm to have a linear relative speedup, due to a good load balance on the processes - as long as $p \leq\ m$  (columns of the matrix $A$). Generating the sub-input is manageable in $O\left(\frac{m \cdot n}{p}\right)$. The complexity of the multiplication one process has to calculate is in $\frac{m \cdot n}{p}$. The complexities of all MPI-functions must be added. MPI\_Allgather,  MPI\_Reduce\_scatter run in $O(n + \log(p))$. As for MPI\_Reduce\_scatter, the input size is dependent on n (number of rows in matrix $A$), so $(n + \log(p))$ has to be added. As for MPI\_Allgather, the input size is dependent on $p$, so $(p + \log(p))$ has to be added as well. After simplification, the complexity should be $O\left(\frac{m \cdot n}{p} + n + p\right)$.\\
